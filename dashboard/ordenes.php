      ...
      <div class="row">
        <div class="col m12 s12">
          <table>
            ...
            <tbody>
              <?php
              $Consulta_Detalle_Factura ="SELECT T1.[ItemCode],T1.[Dscription],T1.[Quantity],T1.[Price],T1.[Quantity]*T1.[Price] FROM ORDR T0  INNER JOIN RDR1 T1 ON T0.DocEntry = T1.DocEntry INNER JOIN OSLP T2 ON T0.SlpCode = T2.SlpCode WHERE T2.[U_CODIGO_USA] = ".$_SESSION['Usuario_Actual']." AND T0.[CANCELED] = 'N' AND T0.[DocNum] = ".$_SESSION['valor_detalle']."";
      ...
      <div class="row">
        <div class="col m12 s12">
          <table>
            ...
            <tbody>
              <?php
              $Consulta_Totales_Detalle ="SELECT T0.[Comments] FROM ORDR T0  INNER JOIN RDR1 T1 ON T0.DocEntry = T1.DocEntry INNER JOIN OSLP T2 ON T0.SlpCode = T2.SlpCode WHERE T2.[U_CODIGO_USA] = ".$_SESSION['Usuario_Actual']." AND T0.[CANCELED] = 'N' AND T0.[DocNum] = ".$_SESSION['valor_detalle']."";
              $Resultado_Totales_Detalle = odbc_exec($Conexion_SQL, $Consulta_Totales_Detalle);
              while (odbc_fetch_array($Resultado_Totales_Detalle)) {
                echo "<tr>";
