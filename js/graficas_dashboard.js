$(document).ready(function(){
   ...
   var Mes_Back_Order = Valores_Back_Order.split(",");                          //Se divide la cadena en partes para poder pasarlos a Float
         ...
         var ctx = document.getElementById("canvas").getContext("2d"); window.myBar = new Chart(ctx).Bar(inicial, {responsive : true ,
           ...
           tooltipTemplate: function(label) { return '$' + label.value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");}}); //Se muestra la gráfica inicial en el dashboard
         $('#facturas_clientes').on('click', function() {                       //Funcion del botón Facturas de Clientes
           $('#Titulo_Grafica').text("Facturas de Clientes");                   //Se cambia el titulo de la gráfica
             ...
             var ctx = document.getElementById("canvas").getContext("2d"); window.myBar = new Chart(ctx).Bar(inicial, {responsive : true ,
             scaleLabel: function(label) { return '$' + label.value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");  },
           tooltipTemplate: function(label) { return '$' + label.value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");}}); }); //Se crea la nueva gráfica
